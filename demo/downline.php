...
function getusers($snode){
    global $s_dbid,$pid,$cnt,$sno,$records_ref,$date1,$date2,$s_package;
        ...
        $sql  = "SELECT id,name,username,email,phone,sponsor,DATE_FORMAT(jdate, '%d-%m-%Y'),dreferal,package FROM `join_store` WHERE `sponsor` = '$snode' order by id";  
...
<br>
  ...
  $sql  = "SELECT `join`.id,name,username,email,phone,sponsor,DATE_FORMAT(jdate, '%d-%m-%Y'),dreferal,investment.plan FROM `join_store` LEFT JOIN investment ON `join`.id = investment.mid WHERE `sponsor` = '$username' order by investment.id;";
  $result = mysqli_query($s_dbid,$sql);
  $cnt=1;
...
$sno =0;
  ...
  $sql  = "SELECT `join`.id,name,username,email,phone,sponsor,DATE_FORMAT(jdate, '%d-%m-%Y'),dreferal,investment.plan FROM `join_store` LEFT JOIN investment ON `join`.id = investment.mid WHERE `sponsor` = '$username' group by username;";
  $result = mysqli_query($s_dbid,$sql);
  $cnt=1;
