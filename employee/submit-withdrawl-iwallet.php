...
//////////// Check Balance Start //////////////
$sql = "SELECT SUM(`comm`) FROM inv_transactions WHERE mid = '$jid' and status != 'Unconfirmed' and `remarks` != 'Wallet Transfer ' and `remarks` NOT LIKE 'iWallet%' and `remarks` NOT LIKE 'Token%' and `remarks` NOT LIKE 'Monthly Cashback Income' and `remarks` NOT LIKE 'Monthly Investment Income'and `remarks` NOT LIKE 'VTC%' ";
$result = @mysqli_query($s_dbid,$sql);
list($p_amt) = @mysqli_fetch_row($result);
$p_amt = $p_amt - ($p_amt*10)/100;

$sql = "SELECT SUM(`comm`) FROM inv_transactions WHERE mid = '$jid' and (`remarks`='Wallet Transfer ' or `remarks` = 'Fund Transfer from Admin' or `remarks` like 'VTC%')";
$result = @mysqli_query($s_dbid,$sql);
list($p_amt2) = @mysqli_fetch_row($result);

$sql = "SELECT SUM(`comm`) FROM inv_transactions WHERE mid = '$jid' and (`remarks`='iWallet Deposit Bonus' or `remarks` = 'iWallet Deposit Weekly Payout' or `remarks` = 'iWallet Deposit Trading Incentive') and status='Confirmed'";
$result = @mysqli_query($s_dbid,$sql);
...
$sql = "SELECT sum(comm) FROM inv_transactions  WHERE inv_transactions.mid = '$jid' and inv_transactions.status != 'Unconfirmed'  and `remarks` in ('Monthly Cashback Income','Monthly Investment Income')";
$result = @mysqli_query($s_dbid,$sql);
list($p_amt4) = @mysqli_fetch_row($result);
$p_amt4 = $p_amt4 - ($p_amt4*5)/100;
//echo $p_amt4;
$sql = "SELECT SUM(`comm`) FROM inv_transactions left join `investment` on inv_transactions.mid = investment.mid WHERE inv_transactions.mid = '$jid' and inv_transactions.status != 'Unconfirmed' and plan = 'voucher' and `remarks` LIKE 'Monthly Cashback Income' and investment_id IS NULL";
$result = @mysqli_query($s_dbid,$sql);
...
$sql = "SELECT sum(`amount`) FROM withdraw WHERE `jid`='$jid' and `wtype` not like 'iWallet%' and `wtype` not like 'Token';";
$result = mysqli_query($s_dbid,$sql);
...
if($wallet_bal>=$amount){
		...
		$pass_sql = "INSERT INTO `withdraw` (`id`, `jid`, `amount`, `status`, `wdate`, `account`, `txn_id`, `wtype`, `amount_ded`,`remark`) VALUES (NULL, '$jid', '$amount', 'Completed', '$wdate', NULL, 'NULL', 'Wallet', '$amount_ded','withdrawal by user to iWallet')"; 
		$pass_result = mysqli_query($s_dbid,$pass_sql);
		 
		
		$sql  = "INSERT INTO `inv_transactions` (`id`, `mid`, `comm`, `amt`, `remarks`, `ttime`, `level`, `rname`, `tleft`, `tright`, `status`) VALUES (NULL, '$jid', '$amount', '0', 'iWallet Transfer', '$wdate', NULL, '$username', NULL, NULL, 'Confirmed');";
	    $result = @mysqli_query($s_dbid,$sql);
  ...
  <tbody>
        ...
        <tbody>
          <tr>
            <td style='font-family: Market Place Path Arial, sans-serif; color: #525151;'><p>Dear ".$name.",<br>
              <br>
              A withdrawal request for ".$amount." to your iWallet has been successfully processed.
    ...
    <tr>
      <td height='32' align='center' valign='middle' style='font-family: Market Place Path Arial, sans-serif; font-size: 12px; color: #555353;'>Copyright &copy; 2021 Spatto Travels. All rights reserved.</td>
		   ...
		   $msg = 'Withdrawal to iWallet Request has been successfully processed.';
	//}
	//else{
