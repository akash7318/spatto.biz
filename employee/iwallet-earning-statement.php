...
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        My Earning
        <small>iWallet Earning Statement</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">iWallet Earning Statement</li>
      </ol>
    </section>
	...
	<div  style="background: #fff">
        <div class="box-header with-border">
         <h3 class="box-title">iWallet Earning Statement</h3>
        </div>
...
<br><br>
      ...
      <tbody>
		      <?php
		        ...
		        $ttds = 0;
		        if($date1==$date2){
		            $sql = "select `join`.name,`join`.username,`comm`,`level`,`remarks`,`ttime`,`rname`,`status` from `join`,inv_transactions where `join`.id = '$mid' and `join`.id = mid and `remarks` IN ('iWallet Deposit Weekly Payout','iWallet Deposit Bonus')  order by inv_transactions.ttime";
		        }
		        else{
		            $sql = "select `join`.name,`join`.username,`comm`,`level`,`remarks`,`ttime`,`rname`,`status` from `join`,inv_transactions where `join`.id = '$mid' and `join`.id = mid and ttime >='$date1' and ttime <= '$date2' and `remarks` IN ('iWallet Deposit Weekly Payout','iWallet Deposit Bonus')  order by inv_transactions.ttime"; 
		        }
		        ...
		        $ttds += $tds;
		        if($remarks=="Cashback Income " || $remarks=="Round Table Bonus"){
		        $rname = $username;
		        $namer = $username;
		        }
		        else{
