...
function getpos(){
    ...
    $sql  = "SELECT id,sponsor,position FROM `join_agent` order by id DESC limit 1";
    ...
    if($lpos<3){
        ...
        $sql  = "SELECT id FROM `join_agent` where username = '$lsponsor'";
        $result = mysqli_query($s_dbid,$sql);
        list($lid) = mysqli_fetch_row($result);
        
        $lid++;
        
        $sql  = "SELECT username FROM `join_agent` where id = '$lid'";
        $result = mysqli_query($s_dbid,$sql);
...
function find_position($snode,$pos) {
      global $s_dbid,$downlineuser,$position;
        ...
        $sql  = "SELECT username FROM `join_agent` WHERE `sponsor` = '$snode' and `position` = '$pos'";
        $result = mysqli_query($s_dbid,$sql);
...
function getNodeInsertPostionByParentId($parentId){
        ...
        foreach($parentId as $parent){  
                ...
                $qry = "select username,position from `join_agent` where sponsor ='".$parent."' order by position";
...
//     }
    ...
    $sql  = "SELECT * FROM `join_agent` WHERE `phone` = '$phone'";
    $result = mysqli_query($s_dbid,$sql);
    $nrows=0;
    $nrows = mysqli_num_rows($result);    
    if ((int)$nrows>0){
    ...
    $loginsql  = "SELECT * FROM `join_agent` WHERE `email` = '$email'";
    ...
    $sql  = "SELECT * FROM `join_agent` WHERE `username` = '$direct'";
    $result = mysqli_query($s_dbid,$sql);
    $nrows=0;
    $nrows = mysqli_num_rows($result);		
    if ((int)$nrows<=0){
...
//echo $flag."test";
		...
		$msg="";
			...
			$sqlc  = "SELECT username FROM `join_agent` order by id desc";
			...
			$sql = "INSERT INTO `join_agent` VALUES (NULL, '$name', '$phone', '$email', '$direct', '$username', '$password', NULL, '$direct',  NULL, 'pending', '$jdate', '$tpassword',  '$title', '$country', '$state', '$city', '$address','$pincode','$area');";
			$result = mysqli_query($s_dbid,$sql);
			
			
			$sql  = "SELECT * FROM `join_agent` WHERE `username` = '$username'";
        	$result = mysqli_query($s_dbid,$sql);
			list($jid) = mysqli_fetch_row($result);
            
            $sql_bank = "INSERT INTO `bank_agent` (`id`, `bank_name`, `branch_name`, `account_no`, `account_type`, `aname`, `jid`, `ifsc`, `pancard`, `paytm`, `phonepe`, `googlepay`, `upi`, `panimage`, `afront`, `aback`, `idnumber`, `idtype`, `panstatus`, `idstatus`, `bank_status`) VALUES (NULL, NULL, NULL, NULL, NULL, NULL, '$jid', NULL, '$pan', NULL, NULL, NULL, NULL, NULL, NULL, NULL, '$aadhaar', NULL, NULL, NULL,NULL);";
  ...
  <tbody>
        ...
        <tbody>
          <tr>
            <td style='font-family: Market Place Path Arial, sans-serif; color: #525151;'><p>Dear ".$name.",<br>
              <br>
              Welcome to Spatto. We have recieved your request for Agent Registration.<br><br>
    ...
    <tr>
      <td height='32' align='center' valign='middle' style='font-family: Market Place Path Arial, sans-serif; font-size: 12px; color: #555353;'>Copyright &copy; 2022 Spatto Services Pvt. Ltd.. All rights reserved.</td>
			...
			mail($to,"Agent Registration - Spatto",$matter,$headers);
...
$message = "Dear ".$name." You have Successfully Registered. Your User ID is ".$username." Password is ".urlencode($password)." Thank You for joining with us. Spatto www.spatto.biz";
