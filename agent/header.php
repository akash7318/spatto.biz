...
$sql = "select `id`,DATE_FORMAT(`jdate`, '%Y') from `join` where `username` = '$username';";           
$result = @mysqli_query($s_dbid,$sql);
list($jid,$adate) = @mysqli_fetch_row($result);

$sql = "select `id`,`package`,`misc`,DATE_FORMAT(`jdate`, '%d-%m-%Y') as jdate,`name`,`email`,`phone`,`sponsor` from `join` where `username` = '$username';";
$result = @mysqli_query($s_dbid,$sql);
list($mid,$package,$status,$jdate,$name,$email,$phone,$s_username) = @mysqli_fetch_row($result);

$sql = "select `name`,`email`,`phone` from `join` where `username` = '$s_username';";
$result = mysqli_query($s_dbid,$sql);
...
function getlvls($snode,$enode){
	...
	$sql1  = "SELECT sponsor FROM `join` WHERE `username` = '$snode'";
        $result1 = mysqli_query($s_dbid,$sql1);
...
function getusers_direct($snode){
    global $s_dbid,$pid,$cnt,$sno,$records_ref;
        ...
        $sql  = "SELECT id,name,username,email,phone,sponsor,DATE_FORMAT(jdate, '%d-%m-%Y'),status,package,dreferal FROM `join` WHERE `dreferal` = '$snode' ";
        $result = mysqli_query($s_dbid,$sql);
...
$boardsql  = "SELECT count(id) FROM `join` WHERE `dreferal` = '$username' and misc = 'active'";
$bresult = @mysqli_query($s_dbid,$boardsql);
...
<div class="wrapper">
  ...
  <header class="main-header">
    ...
    <a href="index2.html" class="logo">
      ...
      <span class="logo-lg"><b>Spatto</b>Services</span>
    </a>
