	...
	<div  style="background: #fff">
      ...
      <tbody>
		      <?php
		        ...
		        $ttds = 0;
		        if($date1==$date2){
		            $sql = "select `join`.name,`join`.username,`comm`,`level`,`remarks`,`ttime`,`rname`,`status`,`investment_id` from `join`,inv_transactions where `join`.id = '$mid' and `join`.id = mid and (`remarks` NOT LIKE 'iWallet Fund Transfer from Admin%' and remarks NOT IN ('iWallet Funds','iWallet Transfer','Wallet Transfer'))  order by inv_transactions.ttime";
		        }
		        else{
		            $sql = "select `join`.name,`join`.username,`comm`,`level`,`remarks`,`ttime`,`rname`,`status`,`investment_id` from `join`,inv_transactions where `join`.id = '$mid' and `join`.id = mid and ttime >='$date1' and ttime <= '$date2' and (`remarks` NOT LIKE 'iWallet Fund Transfer from Admin%' and remarks NOT IN ('iWallet Funds','iWallet Transfer','Wallet Transfer'))  order by inv_transactions.ttime"; 
		        }
		        
		        $result = mysqli_query( $s_dbid, $sql );
		        while (list( $namet, $usernamet, $amt, $level, $remarks, $tdate,$rname,$status,$investment_id ) = mysqli_fetch_row( $result )) {
		        $tamt += $amt;	
		        $sql1 = "select name from `join` where username = '$rname'";
		        $result1 = mysqli_query( $s_dbid, $sql1 );
		        list($namer) = mysqli_fetch_row( $result1 );
		        
		        $sql2 = "select plan from `investment` where mid = '$mid'";
		        $result2 = mysqli_query( $s_dbid, $sql2 );
		        list($plan) = mysqli_fetch_row( $result2 );
		        
		        if($remarks!="Wallet Transfer "){
		            $cdate = date("Y-m-d","2021-11-01");
    		        if($tdate>=$cdate && $remarks=="Monthly Cashback Income" && $plan!="voucher"){
    		            $damt = $amt - ($amt*5)/100;
    		            $tds = ($amt*5)/100;
    		            $admin = 0;
    		        }
    		        else{
    		        if($investment_id!="" || $remarks=="Monthly Investment Income"){
    		            $damt = $amt - ($amt*5)/100;
        		      	$tds = ($amt*5)/100;
        		      	$admin = 0;    
    		        }
		      ...
		      //	  $tds = ($amt*5)/100;
		        ...
		        if($remarks=="Cashback Income" || $remarks=="Round Table Bonus" || $remarks=="iWallet Deposit Trading Incentive" || $remarks == "iWallet Deposit Weekly Payout" || $remarks == "Monthly Investment Income" || $remarks=="Monthly Cashback Income"){
		        $rname = $usernamet;
		        $namer = $namet;
		        }
		        else{
