...
$sno =0;
  ...
  $sql  = "SELECT `join`.id,name,username,email,phone,sponsor,DATE_FORMAT(jdate, '%d-%m-%Y'),dreferal,investment.plan FROM `join` LEFT JOIN investment ON `join`.id = investment.mid WHERE `sponsor` = '$username' group by username;";
  $result = mysqli_query($s_dbid,$sql);
  $cnt=1;
